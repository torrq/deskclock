...
def display_time():
    ...
    while True:
        ...
        if len(time_str) == 6:  # Ensure the time string has exactly 6 characters
            # Update the first row (first set of displays): Display each digit of time_str
            for i in range(6):  # Use 6 because time_str always has 6 characters
                value = int(time_str[i])  # Extract individual digits
                # Send to the corresponding digit in the first row only
